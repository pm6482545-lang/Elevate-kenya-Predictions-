...
export default function SeniorSchoolPage() {
  ...
  const gradeTypes = ['Grade 10', 'Grade 11', 'Grade 12'];
    ...
    const autoFetchFromBucket = async () => {
      ...
      try {
        ...
        if (fileList && Array.isArray(fileList)) {
          ...
          // SORTING LOGIC: Ensures the newest uploaded files always hit the top of the feed
          ...
          const decodedPapers = sortedFileList
          ...
          // Filtering specifically for senior_school tier matching current selection
          const liveFiltered = decodedPapers.filter(
            (p) => p.school_tier === 'senior_school' && 
                   p.grade_class === selectedGrade.toLowerCase().replace(/\s+/g, '') && 
  ...
  const triggerWhatsAppPayment = () => {
    ...
    const customMessage = `Hello Elevate Kenya, I would like to buy the premium Senior School booklet:\n\n` +
                          `• Class Level: ${selectedGrade}\n` +
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F9] text-gray-800 font-sans antialiased">
      ...
      <header className="bg-[#002D62] text-white py-6 sm:py-8 px-4 border-b-4 border-[#D4AF37] text-center shadow-md flex flex-col items-center justify-center gap-2 relative">
        ...
        <div>
          <h1 className="text-xl sm:text-3xl font-black uppercase tracking-tight text-[#D4AF37]">Senior School Hub</h1>
          <p className="text-[10px] sm:text-xs text-gray-300 mt-1 uppercase tracking-widest font-semibold">Grade 10 — Grade 12 National Vault</p>
        </div>
      </header>

      <main className="max-w-4xl mx-auto px-3 sm:px-4 py-6 sm:py-12">
        {/* DIAGNOSTIC ERROR STRIP */}
        {systemError && (
          ...
          <div className="grid grid-cols-3 sm:inline-flex rounded-lg bg-gray-200 p-1 shadow-inner w-full sm:w-auto gap-1 sm:gap-0">
            {gradeTypes.map((grade) => (
              <button 
                ...
                className={`px-2 sm:px-6 py-2.5 rounded-md text-xs font-black tracking-wider uppercase transition-all text-center ${selectedGrade === grade ? 'bg-[#002D62] text-white shadow-md' : 'text-gray-600'}`}
              >
                {grade}
              </button>
            ))}
        ...
        {/* INTERACTIVE DUAL CHECKOUT DRAWER */}
            ...
            <div className="flex justify-between items-start mb-4 border-b pb-3">
              <div>
                ...
                <p className="text-sm font-extrabold text-gray-900 mt-1 uppercase">
                  {selectedGrade} {checkoutPaper.subject} — <span className="text-[#D4AF37]">KES {checkoutPaper.price}</span>
                </p>
              </div>
              ...
              <div className="bg-gray-50/50 border border-gray-100 rounded-xl p-4 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 leading-normal mb-4">
                    Need instant customer clearing parameters or specific assistance? Connect immediately to our desk operations.
                  </p>
                </div>
                <button 
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm">
          <div className="flex justify-between items-center border-b pb-3 mb-4">
            <h3 className="font-black text-[#002D62] text-xs sm:text-sm uppercase tracking-wider">
              Available {selectedGrade} Booklets ({selectedTerm})
            </h3>
            <span className="text-[9px] font-bold bg-[#002D62]/10 text-[#002D62] px-2 py-0.5 rounded uppercase tracking-wider">
              ⏱️ Newest First
            </span>
          </div>
