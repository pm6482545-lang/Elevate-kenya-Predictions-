...
export default function JuniorSchoolPage() {
  ...
  const gradeTypes = ['Grade 7', 'Grade 8', 'Grade 9'];
    ...
    const autoFetchFromBucket = async () => {
      ...
      try {
        ...
        if (fileList && Array.isArray(fileList)) {
          ...
          // Filtering specifically for junior_school tier matching the selected state values
          const liveFiltered = decodedPapers.filter(
            (p) => p.school_tier === 'junior_school' && 
  ...
  // AUTOMATED PRE-FILLED WHATSAPP MESSAGE ROUTING TO YOUR DESK
  const triggerWhatsAppPayment = () => {
    const businessPhone = "254746357349"; 
    const customMessage = `Hello Elevate Kenya, I would like to buy the premium Junior School booklet:\n\n` +
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F9] text-gray-800 font-sans antialiased">
      {/* MOBILE-RESPONSIVE THEMED HEADER */}
      <header className="bg-[#002D62] text-white py-6 sm:py-8 px-4 border-b-4 border-[#D4AF37] text-center shadow-md flex flex-col items-center justify-center gap-2 relative">
        ...
        <div>
          <h1 className="text-xl sm:text-3xl font-black uppercase tracking-tight text-[#D4AF37]">Junior School Hub</h1>
          <p className="text-[10px] sm:text-xs text-gray-300 mt-1 uppercase tracking-widest font-semibold">Grade 7 — Grade 9 Resource Vault</p>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-3 sm:px-4 py-6 sm:py-12">
        ...
        {/* INTERACTIVE DUAL PAYMENT DRAWER */}
              ...
              <div className="bg-gray-50/50 border border-gray-100 rounded-xl p-4 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 leading-normal mb-4">
                    Prefer interacting directly or manual business payment configuration? Connect instantly with our service desk.
                  </p>
                </div>
                <button 
