...
export default function UpperPrimaryPage() {
  ...
  const gradeTypes = ['Grade 4', 'Grade 5', 'Grade 6'];
    ...
    const autoFetchFromBucket = async () => {
      ...
      try {
        ...
        if (fileList && Array.isArray(fileList)) {
          ...
          // Filtering specifically for upper_primary tier matching the compact lowercase space strings
          const liveFiltered = decodedPapers.filter(
            (p) => p.school_tier === 'upper_primary' && 
  ...
  const triggerWhatsAppPayment = () => {
    ...
    const customMessage = `Hello Elevate Kenya, I would like to buy the premium Upper Primary booklet:\n\n` +
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F9] text-gray-800 font-sans antialiased">
      ...
      <header className="bg-[#002D62] text-white py-6 sm:py-8 px-4 border-b-4 border-[#D4AF37] text-center shadow-md flex flex-col items-center justify-center gap-2 relative">
        ...
        <div>
          <h1 className="text-xl sm:text-3xl font-black uppercase tracking-tight text-[#D4AF37]">Upper Primary Hub</h1>
          <p className="text-[10px] sm:text-xs text-gray-300 mt-1 uppercase tracking-widest font-semibold">Grade 4 — Grade 6 Assessment Sets</p>
        </div>
      </header>
              ...
              <div className="bg-gray-50/50 border border-gray-100 rounded-xl p-4 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 leading-normal mb-4">
                    Prefer direct messaging or manual clearance? Transfer parameters manually using our fast help channel.
                  </p>
                </div>
                <button 
